  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        built: true,
        // `label.print` alone, not either-of. Every read the screen makes --
        // the barcode scheme and the layouts -- is gated on print, so a custom
        // role holding only `label.manage` would see the link and get a 403 on
        // the first request. Manage is what unlocks editing once inside; it is
        // not a way in. No seeded role holds it without print, but the role
        // builder can make one.
        permissions: ['label.print'],
