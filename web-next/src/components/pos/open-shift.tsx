    ...
    <div className="grid flex-1 place-items-center p-4">
      <form onSubmit={submit} className="w-full max-w-sm">
        ...
        <Field
          label={
            currency
              ? t('nx.pos.openingFloat', { currency })
              : t('nx.pos.openingFloatBare')
          }
          hint={t('nx.pos.floatHint')}
          required
        >
          <Input
            value={float}
        ...
        <Button
          ...
          className="mt-6 h-12"
          busy={busy}
          busyLabel={t('nx.pos.openingCounter')}
        >
          {t('nx.pos.startSelling')}
        </Button>
      </form>
    </div>
  );
