...
| # | Problem | Why it matters | Fix |
|---|---|---|---|
| **P20** | **Sign-in is ambiguous when one email exists in two tenants.** `app_user_email_tenant_uq` makes email unique *within* a tenant, which is correct for multi-tenant SaaS — the same bookkeeper legitimately serves two shops. But `identity.Authenticate` looks the account up with `WHERE email = $1` and no tenant qualifier, via `QueryRow`. With two matching rows it silently takes one, and the other user can never sign in. Found by a browser check: an account authenticated over curl and was refused in the browser moments later, because two tenants shared the address. | Any two tenants whose users share an email — a franchise owner, an accountant with several clients, a shared ops address. One is locked out with "that email or password is not correct", which is untrue and undiagnosable from outside. | Needs a tenant-scoping decision, not a patch: a subdomain per tenant, a tenant picker after an ambiguous email, or global email uniqueness. Deliberately NOT chosen here — it is an authentication architecture change and belongs in its own milestone. |
