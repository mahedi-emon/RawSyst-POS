...
| **Branch** | `main` @ `07adf9f` |
| **Backend** | 136 Go files, ~44,000 lines, 822 integration tests + 234 unit tests, 51 migrations |
...
| **P33** | **A drawer variance is recorded but never posted.** Design 11 §9 says the Z-report variance "posts to a Cash Over/Short account rather than being absorbed silently". It does not: `shift.Close` writes `variance` onto `cash_session` and calls nothing in `accounting`, and no `cash_over_short` role or posting rule exists anywhere in the schema. Found while verifying the till screens against the design. | The difference is visible on the Z report and on the shift record, so nothing is hidden from a person reading it — but it never reaches the ledger, so a month of short drawers does not appear in the accounts and the cash account carries a balance the drawer never held. | A posting rule plus a `cash_over_short` account role and a chart mapping, posted from `Close` in the same transaction that freezes the count. Deliberately NOT done as part of the till screens: it is an accounting change, and P32 already shows what happens when a rule names a role the chart does not map. |
