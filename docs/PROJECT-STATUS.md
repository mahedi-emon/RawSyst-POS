...
| **P34** | **C13's tie-out drifts when a cost carries more than two decimals.** `cost_layer.unit_cost` is `numeric(18,4)` and `inventory_valuation` sums `qty_remaining * unit_cost` at that precision, but `accounting.Post` rounds `base_debit`/`base_credit` to 2 dp (`moneyScale`) and `inventory_gl_difference` compares the two. Receipts at 33.3333 / 16.6667 / 99.9999 value at **716.6663** against a ledger of **716.67**. Reachable in production: `finalize.go` posts `COGSTotal` through the same engine. **Found by P17** — the tie-out harness had been writing its own journal lines at 4 dp, so the valuation was being compared against numbers no production path would ever store, which is the "harness ties out to harness" failure `tieout_test.go`'s own header warns about. | C13 says the valuation must tie EXACTLY and any divergence is an exception. Today a shop using weighted average on awkward purchase prices accrues a sub-hallala difference per sale that never clears. | Genuinely unspecified: design 02 §335 warns that double-rounding breaks the tie-out but does not say which precision wins. Rounding the valuation to 2 dp, carrying 4 dp in the ledger, and constraining layers so `qty x unit_cost` is always 2 dp are three different accounting answers. Needs a decision, not a guess. |
